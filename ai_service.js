...
function getModel(apiKey, useFallback = false) {
    if (_cachedKey !== apiKey) {
        ...
        _fallbackModel = genAI.getGenerativeModel({
            model: "gemini-flash-latest",
            generationConfig: { responseMimeType: "application/json" }
        });
        console.log("[AI Service] Model instances created (cached).");
    }
    return useFallback ? _fallbackModel : _model;
}
...
async function generateTicketMessage(summary, clientName, userApiKey) {
    ...
    console.log(`[AI Service] Generating message for: "${summary}" | Client: "${clientName}"`);
    ...
    const prompt = `Você é um especialista em Service Desk da empresa Hebron.
Crie uma DESCRIÇÃO TÉCNICA concisa para o chamado abaixo.

# Contexto Hebron
- Hebronline: Sistema de visita médica. Usuários: Propagandistas (PGs) ou setores (ex: 10.14.007)
...
{ "descricao": "Texto da descrição" }`;
    ...
    try {
        ...
        if (errStr.includes('404') || errStr.includes('Not Found')) {
            console.warn("⚠️ Primary model not found. Fallback to 'gemini-flash-latest'...");
            try {
                const fallbackModel = getModel(API_KEY, true);
...
# Contexto Hebron
- PGs: Propagandistas. PDV: Ponto de Venda. V.A.: PDF/Portfólio
...
{ "solucao": "Texto da solução" }`;
    ...
    try {
        ...
        if (errStr.includes('404') || errStr.includes('Not Found')) {
            console.warn("⚠️ Solution: Model not found. Fallback to 'gemini-flash-latest'...");
            try {
                const fallbackModel = getModel(API_KEY, true);
                const result = await fallbackModel.generateContent(prompt);
                return cleanResponse(result.response.text());
